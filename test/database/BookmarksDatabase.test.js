...
describe("getBookmarks", () => {
  ...
  it("success case - several bookmarks exist for the specified user", async () => {
    ...
    for (const title of titles) {
      const existingBookmark = new BookmarkModel({ userId, title });
...
describe("createBookmark", () => {
  it("error case - a bookmark with the same title for the specified user already exists", async () => {
    ...
    await expect(
      bookmarksDatabase.createBookmark(userId, title)
    ).rejects.toThrow(new ConflictingResourceError("Bookmark already exists"));
  });
...
describe("deleteBookmark", () => {
  it("error case - no existing bookmark matches userId", async () => {
    ...
    await bookmarksDatabase.createBookmark(otherUserId, sharedBookmarkTitle);

    const targetUserId = "ADVIL";
    await expect(
      bookmarksDatabase.deleteBookmark(targetUserId, sharedBookmarkTitle)
    ).rejects.toThrow(
  ...
  it("error case - no existing bookmark matches title", async () => {
    ...
    await expect(
      bookmarksDatabase.deleteBookmark(sharedUserId, targetBookmarkTitle)
    ).rejects.toThrow(
      new BookmarkNotFoundError(
  ...
  it("success case", async () => {
    ...
    await bookmarksDatabase.deleteBookmark(user1Id, title);

    const bookmarksUser1 = await bookmarksDatabase.getBookmarks(user1Id);
    const bookmarksUser2 = await bookmarksDatabase.getBookmarks(user2Id);
